...
# To display graph and model on tensorboard please type  :::::::: >> tensorboard --logdir=./tmp/data/1
...
show_steps = 29
logdir = "./tmp/data/1"
file_handle = open("C:/Users/Devilal/Documents/Machine Learning/Data/emnist-0A/captured_data.csv", "r")
reader = csv.reader(file_handle, delimiter = ',')


#file_handle_test = open("C:/Users/Devilal/Documents/Machine Learning/Data/emnist-0A/captured_data.csv", "r")
...
def train_model():
	...
	if(pretrained == False):
		...
		#build model
		resultOp, crossEntropy, optimizer, prediction = m.model(input_x, input_y)

		saver = tf.train.Saver()
		tf.add_to_collection("input_x", input_x)
		tf.add_to_collection("input_y", input_y)
		tf.add_to_collection("resultOp", resultOp)
		...
		for ep in range(epochs):
			...
			for i in range(nit):
				...
				if((i + 1) % show_steps == 0):
					...
					if(drop_cnt == 15):
						print("Model Accuracy is not increasing since last 15 iterations. So we are interrupting and blocking the training process.")
						iterations += 1
						bflag = True
						break
					total_cost = 0
